...
A caching API for Python services that is loosely modeled after the Java Caching API (JSR107_).
...
Unless overridden via the `cache_name` parameter ``slycache`` will
...
To avoid having to specify the `cache_name` parameter on each call you can
...
`Python format strings <https://docs.python.org/3/library/string.html#format-string-syntax>`_.
